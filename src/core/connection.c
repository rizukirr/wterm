...
void init_connection_cancel(void) { connection_cancelled = 0; }

void request_connection_cancel(void) { connection_cancelled = 1; }

bool is_connection_cancelled(void) { return connection_cancelled != 0; }
...
static connection_result_t execute_nmcli_connect(const char *command,
  ...
  for (int i = 0; i < 13; i++) {
    ...
    // This catches cases where NM reports failure but device is actually
    // connected However, we also need to ensure DHCP completed and IP was
    // assigned
    char iw_ssid[MAX_STR_SSID];
    if (iw_get_connected_ssid(wifi_interface, iw_ssid, sizeof(iw_ssid)) ==
        WTERM_SUCCESS) {
      if (strlen(iw_ssid) > 0 && strcmp(iw_ssid, ssid) == 0) {
        // WiFi is associated, now check if IP address is assigned
        bool has_ip = false;
        if (interface_has_ip_address(wifi_interface, &has_ip) ==
                WTERM_SUCCESS &&
            has_ip) {
          result.result = WTERM_SUCCESS;
          result.connected = true;
      ...
      while (fgets(check_buffer, sizeof(check_buffer), check_fp)) {
        ...
        if (colon) {
          *colon = '\0';
          ...
          // Check if this connection is for our SSID and in failed/deactivated
          // state
  ...
  if (exit_code != 0 && strlen(error_output) > 0) {
    ...
    snprintf(result.error_message, sizeof(result.error_message),
             "Connection to %s timed out (check signal strength, password, or "
             "AP availability)",
             ssid);
  }
  result.connected = false;
 ...
 *
 * @return wterm_result_t WTERM_SUCCESS if cleanup successful or no cleanup
 * needed
 */
static wterm_result_t cleanup_zombie_before_connect(void) {
  ...
  if (device_pid == 0) {
    ...
    char *const device_args[] = {"nmcli", "device", "disconnect",
  ...
  if (still_associated) {
    char *const unmanage_args[] = {
        "nmcli",   "device", "set", (char *)wifi_interface,
        "managed", "no",     NULL};
...
connection_result_t connect_to_open_network(const char *ssid) {
  ...
  if (cleanup_result != WTERM_SUCCESS) {
    result.result = WTERM_ERROR_NETWORK;
    safe_string_copy(result.error_message,
                     "Failed to cleanup zombie connection before connecting",
                     sizeof(result.error_message));
    return result;
  }
...
connection_result_t connect_to_secured_network(const char *ssid,
  ...
  if (connection_exists(ssid)) {
    ...
    snprintf(command, sizeof(command),
             "nmcli device wifi connect %s password %s 2>&1", escaped_ssid,
             escaped_password);
  }
...
connection_status_t get_connection_status(void) {
  ...
  if (kernel_associated) {
    iw_get_connected_ssid(wifi_interface, status.kernel_ssid,
                          sizeof(status.kernel_ssid));
  }
  ...
  if (status.is_zombie) {
    ...
    if (recover_from_zombie_connection(wifi_interface) == WTERM_SUCCESS) {
      ...
      if (status.is_connected) {
        ...
        fp = popen("nmcli -t -f IP4.ADDRESS connection show --active "
                   "2>/dev/null | head -1",
                   "r");
        if (fp) {
          if (fgets(buffer, sizeof(buffer), fp)) {
...
wterm_result_t disconnect_current_network(void) {
  ...
  // This ensures we disconnect even if NetworkManager has no active connection
  // profile but the WiFi device still has a kernel-level association Note: This
  // may fail if NM already considers the device disconnected (expected for
  // zombies) We suppress stderr to avoid confusing error messages for expected
  // failures
  pid_t device_pid = fork();
  if (device_pid == 0) {
    ...
    // Parent: wait for child but ignore result (failure is expected for
    // zombies)
    int status;
    waitpid(device_pid, &status, 0);
  }

  // Verify disconnection by checking if any WiFi association remains at kernel
  // level
...
wterm_result_t recover_from_zombie_connection(const char *interface) {
  ...
  char *const unmanage_args[] = {"nmcli",   "device", "set", (char *)interface,
                                 "managed", "no",     NULL};
  ...
  char *const manage_args[] = {"nmcli",   "device", "set", (char *)interface,
                               "managed", "yes",    NULL};
