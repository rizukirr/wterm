...
typedef struct {
  ...
  backend_result_t (*connect_secured_network)(const char *ssid,
                                              const char *password);
  ...
  wterm_result_t (*get_hotspot_status)(const char *hotspot_name,
                                       hotspot_status_t *status);
  wterm_result_t (*list_active_hotspots)(char active_hotspots[][MAX_STR_SSID],
                                         int max_count, int *count);
  wterm_result_t (*get_hotspot_clients)(const char *hotspot_name,
                                        hotspot_client_t *clients,
                                        int max_clients, int *client_count);

  // Interface capability checking
  wterm_result_t (*check_interface_ap_support)(const char *interface,
                                               bool *supports_ap);
  wterm_result_t (*get_available_wifi_interfaces)(
      char interfaces[][MAX_STR_INTERFACE], int max_interfaces,
      int *interface_count);
} network_backend_t;
