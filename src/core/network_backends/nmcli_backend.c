...
static backend_result_t nmcli_connect_secured_network(const char* ssid, const char* password);
...
static wterm_result_t nmcli_get_hotspot_status(const char* hotspot_name, hotspot_status_t* status);
static wterm_result_t nmcli_list_active_hotspots(char active_hotspots[][MAX_STR_SSID], int max_count, int* count);
static wterm_result_t nmcli_get_hotspot_clients(const char* hotspot_name, hotspot_client_t* clients, int max_clients, int* client_count);
static wterm_result_t nmcli_check_interface_ap_support(const char* interface, bool* supports_ap);
static wterm_result_t nmcli_get_available_wifi_interfaces(char interfaces[][MAX_STR_INTERFACE], int max_interfaces, int* interface_count);
...
static bool nmcli_is_available(void) {
    return (system("command -v nmcli >/dev/null 2>&1") == 0);
}
...
static backend_result_t execute_nmcli_command(const char* command) {
    ...
    if (exit_code != 0) {
        ...
        if (strlen(error_output) > 0) {
            ...
            safe_string_copy(result.error_message, "Command failed", sizeof(result.error_message));
        }
    }
...
static backend_result_t nmcli_connect_open_network(const char* ssid) {
    ...
    if (!ssid) {
        ...
        safe_string_copy(result.error_message, "Invalid SSID", sizeof(result.error_message));
        return result;
    }
    ...
    char command[512];
    snprintf(command, sizeof(command), "%s %s 2>&1", NMCLI_WIFI_CONNECT, escaped_ssid);
...
static backend_result_t nmcli_connect_secured_network(const char* ssid, const char* password) {
    ...
    if (!ssid || !password) {
        result.result = WTERM_ERROR_INVALID_INPUT;
        safe_string_copy(result.error_message, "Invalid SSID or password", sizeof(result.error_message));
        return result;
    }
...
static wterm_result_t nmcli_rescan_networks(void) {
    int result = system(NMCLI_WIFI_SCAN);
...
static bool nmcli_get_ip_address(char* ip_buffer, size_t buffer_size) {
    ...
    FILE* fp = popen("nmcli -t -f IP4.ADDRESS connection show --active 2>/dev/null | head -1", "r");
    if (!fp) {
        return false;
    }
...
static backend_result_t nmcli_create_hotspot(const hotspot_config_t* config) {
    ...
    if (!config) {
        result.result = WTERM_ERROR_INVALID_INPUT;
        safe_string_copy(result.error_message, "Invalid hotspot configuration", sizeof(result.error_message));
        return result;
    }
    ...
    if (!validate_interface_name(config->wifi_interface)) {
        result.result = WTERM_ERROR_INVALID_INPUT;
        safe_string_copy(result.error_message, "Invalid WiFi interface name", sizeof(result.error_message));
        return result;
    }

    if (!validate_hotspot_name(config->name)) {
        result.result = WTERM_ERROR_INVALID_INPUT;
        safe_string_copy(result.error_message, "Invalid hotspot name", sizeof(result.error_message));
        return result;
    }
    ...
    if (config->password[0] != '\0' && config->security_type != WIFI_SECURITY_NONE) {
        // Secured hotspot with password
        if (!shell_escape(config->password, escaped_password, sizeof(escaped_password))) {
            result.result = WTERM_ERROR_INVALID_INPUT;
            safe_string_copy(result.error_message, "Password too long for shell escaping", sizeof(result.error_message));
            return result;
        }
    ...
    if (written >= (int)sizeof(command)) {
        ...
        safe_string_copy(result.error_message, "Command too long", sizeof(result.error_message));
        return result;
    }
    ...
    if (config->channel > 0) {
        ...
        if (written >= (int)sizeof(temp)) {
            result.result = WTERM_ERROR_GENERAL;
            safe_string_copy(result.error_message, "Command with channel too long", sizeof(result.error_message));
            return result;
        }
    ...
    if (config->is_5ghz) {
        char temp[2048];
        written = snprintf(temp, sizeof(temp), "%s 802-11-wireless.band a", command);
        if (written >= (int)sizeof(temp)) {
            result.result = WTERM_ERROR_GENERAL;
            safe_string_copy(result.error_message, "Command with band too long", sizeof(result.error_message));
            return result;
        }
        safe_string_copy(command, temp, sizeof(command));
    } else {
        char temp[2048];
        written = snprintf(temp, sizeof(temp), "%s 802-11-wireless.band bg", command);
        if (written >= (int)sizeof(temp)) {
            result.result = WTERM_ERROR_GENERAL;
            safe_string_copy(result.error_message, "Command with band too long", sizeof(result.error_message));
            return result;
    ...
    written = snprintf(command_with_stderr, sizeof(command_with_stderr), "%s 2>&1", command);
    if (written >= (int)sizeof(command_with_stderr)) {
        result.result = WTERM_ERROR_GENERAL;
        safe_string_copy(result.error_message, "Final command too long", sizeof(result.error_message));
        return result;
    }
...
static backend_result_t nmcli_start_hotspot(const char* hotspot_name) {
    ...
    if (!validate_hotspot_name(hotspot_name)) {
        result.result = WTERM_ERROR_INVALID_INPUT;
        safe_string_copy(result.error_message, "Hotspot name contains invalid characters", sizeof(result.error_message));
        return result;
    }

    char command[256];
    // hotspot_name is already validated, safe to use directly
    snprintf(command, sizeof(command), "nmcli connection up %s 2>&1", hotspot_name);
...
static backend_result_t nmcli_stop_hotspot(const char* hotspot_name) {
    ...
    char command[256];
    snprintf(command, sizeof(command), "nmcli connection down %s 2>&1", hotspot_name);
...
static backend_result_t nmcli_delete_hotspot(const char* hotspot_name) {
    ...
    char command[256];
    snprintf(command, sizeof(command), "nmcli connection delete %s 2>&1", hotspot_name);
...
static wterm_result_t nmcli_get_hotspot_status(const char* hotspot_name, hotspot_status_t* status) {
    ...
    snprintf(command, sizeof(command),
            "nmcli -t -f NAME,TYPE,STATE connection show %s 2>/dev/null", hotspot_name);
...
static wterm_result_t nmcli_list_active_hotspots(char active_hotspots[][MAX_STR_SSID], int max_count, int* count) {
    ...
    *count = 0;
    ...
    FILE* fp = popen("nmcli -t -f NAME,TYPE,STATE connection show --active 2>/dev/null", "r");
    if (!fp) {
        return WTERM_ERROR_NETWORK;
    }
...
static wterm_result_t nmcli_get_hotspot_clients(const char* hotspot_name, hotspot_client_t* clients, int max_clients, int* client_count) {
    ...
    *client_count = 0;
    ...
    char command[256];
    snprintf(command, sizeof(command),
            "nmcli -t -f connection.interface-name connection show %s 2>/dev/null", hotspot_name);
    ...
    snprintf(command, sizeof(command), "iw dev %s station dump 2>/dev/null", interface);
    ...
    while (fgets(buffer, sizeof(buffer), fp) && *client_count < max_clients) {
        ...
        if (strncmp(buffer, "Station ", 8) == 0) {
            ...
            if (space_pos) {
                *space_pos = '\0';
                safe_string_copy(clients[*client_count].mac_address, mac_start, MAX_STR_MAC_ADDR);
                // Initialize other fields with default values
                safe_string_copy(clients[*client_count].hostname, "Unknown", MAX_STR_DEVICE_NAME);
                safe_string_copy(clients[*client_count].ip_address, "Unknown", MAX_STR_IP_ADDR);
...
static wterm_result_t nmcli_check_interface_ap_support(const char* interface, bool* supports_ap) {
    ...
    *supports_ap = false;
    ...
    char command[256];
    snprintf(command, sizeof(command), "iw dev %s info 2>/dev/null | grep -q 'type AP'", interface);
    ...
    // Alternative check: see if interface supports creating AP-type interfaces
    snprintf(command, sizeof(command), "iw phy %s info 2>/dev/null | grep -q 'AP'", interface);
    result = system(command);
    *supports_ap = (result == 0);
...
static wterm_result_t nmcli_get_available_wifi_interfaces(char interfaces[][MAX_STR_INTERFACE], int max_interfaces, int* interface_count) {
    ...
    *interface_count = 0;
    ...
    char buffer[128];
    while (fgets(buffer, sizeof(buffer), fp) && *interface_count < max_interfaces) {
        buffer[strcspn(buffer, "\n")] = '\0';
        ...
        if (device_field && type_field && strcmp(type_field, "wifi") == 0) {
            safe_string_copy(interfaces[*interface_count], device_field, MAX_STR_INTERFACE);
            (*interface_count)++;
        }
    }
